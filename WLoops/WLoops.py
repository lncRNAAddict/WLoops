# -*- coding: utf-8 -*-
"""
Created on Wed Jun 10 16:10:03 2026
...
from scipy.stats import expon,kstest,gamma, cramervonmises, poisson,wasserstein_distance,fit, weibull_min
...
#selected_distributions = ['lognorm', 'expon', 'pareto','gamma','weibull_min','weibull_max']
#selected_distributions = ['lognorm', 'expon', 'pareto','gamma','weibull_min','weibull_max']
...
def parse_args(args):
    ...
    parser.add_argument("-B",
                        "--background",
                        dest="background",
                        help="REQUIRED: background distribution, W: weibull , E: exponential ",
                        required=False)
...
def read_cooler(f, distance_in_bp, chr1, chr2, cooler_balance):
    ...
    if chr1 == chr2:
        ...
        while start < CHRM_SIZE:
            ...
            np.nan_to_num(np.asarray(temp), copy=False, nan=0, posinf=0, neginf=0)
        ...
        np.nan_to_num(np.asarray(result), copy=False, nan=0, posinf=0, neginf=0)
        x = result.row
        y = result.col
        val = result.data
...
def inter_normalize_map(vals):
    ...
    cmap -= m
    cmap /= s
    np.nan_to_num(cmap, copy=False, nan=0, posinf=0, neginpval_weightsf=0)

'''
def wassertein_distance(data,fitted):
    #data_sorted = np.sort(data)
    #p = (np.arange(len(data)) + 0.5) / len(data)
    if fitted == 'expon':
        fit_params = expon.fit(data)
        fitted_samples = expon.rvs(*fit_params, size=len(data))
        w1_distance = wasserstein_distance(data, fitted_samples)
        return w1_distance
    elif fitted == 'poisson':
        fit_params = fit(data,'poisson')
        fitted_samples = expon.rvs(*fit_params, size=len(data))
        w1_distance = wasserstein_distance(data, fitted_samples)
        return w1_distance
        

def KS_distance(data,fitted):
    if fitted == 'expon':
        loc,scale= expon.fit(data)
        result = kstest(data, 'expon', args=(loc, scale))
        return result.statistic
    elif fitted == 'poisson':
        loc,scale= poisson.fit(data)
        result = kstest(data, 'expon', args=(loc, scale))
        return result.statistic
    
def cramer_distance(data,fitted):
    if fitted == 'expon':
        loc,scale= expon.fit(data)
        fitted_dist = expon(loc,scale)
        result = cramervonmises(data, fitted_dist.cdf)
        return result.statistic
    elif fitted == 'poisson':
        loc = poisson.fit(data)
        fitted_dist = poisson(loc)
        result = cramervonmises(data, fitted_dist.cdf)
        return result.statistic
...
def mustache(c, chromosome, chromosome2, res, pval_weights, start, end, mask_size, distance_in_px, octave_values, st,
    ...
    for o in octave_values:
        ...
        # Lp = Gp - Gc
        Lc = Gc - Gn
        # to test different distributions
        '''
        df = distfit(distr=selected_distributions)

        df.fit_transform(np.abs(Lc[nz]))
        
        # Plot seperately
        fig, ax = df.plot(chart='pdf')
        #plt.show()
        #fig, ax = df.qqplot(np.abs(Lc[nz]), n_top=2)
        
        plt.show()
        sys.exit(0)
        df.summary.to_csv(chromosome +".fits.summary.csv",mode='a',index=False)
        '''
        ...
        for i in range(3, s + 2):
            ...
            if background_dist == "W":
                dist = distfit(distr=['weibull_min'])
                dist.fit_transform(np.abs(Lc[nz]),verbose = 0)
                dist_params = dist.model['params']
                pval = 1 - weibull_min.cdf(np.abs(Lc[nz]), *dist_params)
            
            elif background_dist == "E":
...
def regulator(f, norm_method, CHRM_SIZE, outdir, bed="",
              ...
              bias=False,
              chromosome='n',
              chromosome2=None,background_dist = "E"):
    ...
    if chromosome == chromosome2:
        ...
        with Manager() as manager:
            ...
            for i in range(len(start)):
                ...
                cc[xc, yc] = vc
                print(cc.shape)
                plt.imshow(cc[200:350,200:350],cmap="Reds")
                plt.colorbar()
                plt.show()
                return
                #             
                p = Process(target=process_block, args=(
                    ...
                    octave_values, o, st, pt,background_dist))
                p.start()
                processes.append(p)
...
def main():
    ...
    print(background)
    if not background:
        print("Error: no background distribution ")
        return
    else:
        if background != "W" and background != "E":
            print("Error: incorrect background distribution, should be E (exponential) or W (Weibull")
            return
    ...
    for i, (chromosome, chromosome2) in enumerate(zip(chr_list, chr_list2)):
        ...
        o = regulator(f, args.norm_method, CHRM_SIZE, args.outdir,
                      ...
                      octaves=args.octaves,background_dist = background)
        if i == 0:
